...
#########################
# G ~ climate all variables  + Structure + Latitude + Longtitude 
#########################
library(vegan)
library(adegenet)
library(psych)
...
cat("Missing values in genotype data before processing:", sum(is.na(genotype_data)), "\n")

# Impute missing values using the most common value
impute_geno <- function(x) {
  x[is.na(x)] <- as.numeric(names(which.max(table(x, useNA = "no"))))
  return(x)
}

genotype_data_clean <- apply(genotype_data, 2, impute_geno)

# Check dimensions after imputation
cat("Final genotype data dimensions:", dim(genotype_data_clean), "\n")

#########################
# Load and Clean Environmental Data
#########################
env_data <- read.csv("~/R/cannabis_GEAV/outputs/env_data_clean2.csv", stringsAsFactors = FALSE)

# Ensure column consistency
colnames(env_data)[1] <- "Sample_ID"

# Check missing values
cat("Missing values in environmental data:", sum(is.na(env_data)), "\n")

#########################
# Load and Merge PCA (Structure Data)
#########################
pca_data <- read.csv("~/R/cannabis_GEAV/MERGE2/soorni_ren_merged_PC.csv", row.names = 1)
colnames(pca_data)[1] <- "Sample_ID"

# Merge PCA data with environmental data
env_data_clean <- merge(env_data, pca_data, by = "Sample_ID", all.x = TRUE)

# Remove duplicate columns from merging (_y suffix)
env_data_clean <- env_data_clean[, !grepl("\\.y$", colnames(env_data_clean))]


#########################
# Align Genotype and Environmental Data
#########################
# Find common sample IDs
common_samples <- intersect(rownames(genotype_data_clean), env_data_clean$Sample_ID)
...
# Ensure they match
cat("Number of samples in genotype data:", nrow(genotype_data_clean), "\n")
cat("Number of samples in environmental data:", nrow(env_data_clean), "\n")

env_data_clean <- env_data_clean[, c("Latitude.x", "Longitude.x",
                                     "bio_01.x", "bio_02.x", "bio_03.x", "bio_04.x",
                                     "bio_05.x", "bio_06.x", "bio_07.x", "bio_08.x",
                                     "bio_09.x", "bio_10.x", "bio_11.x", "bio_12.x",
                                     "bio_13.x", "bio_14.x", "bio_15.x", "bio_16.x",
                                     "bio_17.x", "bio_18.x", "bio_19.x",
                                     "EV1.x", "EV2.x", "EV3.x", "EV4.x")]
...
climate_vars <- env_data_clean[, grep("bio_\\d{2}\\.x", colnames(env_data_clean))]

# Define conditioning variables (structure + location)
condition_vars <- env_data_clean[, c("EV1.x", "EV2.x", "EV3.x", "EV4.x", "Latitude.x", "Longitude.x")]

# Run partial RDA: genotype ~ climate | structure + lat + lon
rda_partial <- rda(genotype_data_clean, climate_vars, condition_vars)
...
plot(rda_partial, scaling = 3)


#########################
#G ~ Structure | (climate all variables + Latitude + Longtitude) 
#########################

# Define structure variables
structure_vars <- env_data_clean[, c("EV1.x", "EV2.x", "EV3.x", "EV4.x")]

# Define conditioning variables (climate + location)
condition_vars <- env_data_clean[, c("Latitude.x", "Longitude.x",
                                     "bio_01.x", "bio_02.x", "bio_03.x", "bio_04.x",
                                     "bio_05.x", "bio_06.x", "bio_07.x", "bio_08.x",
                                     "bio_09.x", "bio_10.x", "bio_11.x", "bio_12.x",
                                     "bio_13.x", "bio_14.x", "bio_15.x", "bio_16.x",
...
# Run partial RDA: genotype ~ structure | climate + location
rda_structure_partial <- rda(genotype_data_clean, structure_vars, condition_vars)

# View summary
summary(rda_structure_partial)

# Adjusted R²
RsquareAdj(rda_structure_partial)

# Permutation test for significance
anova(rda_structure_partial, permutations = 999)
plot(rda_structure_partial, scaling = 3)

#########################
#G ~ Latitude + Longtitude | (climate all variables + Structure)
#########################


# Define geographic variables
geography_vars <- env_data_clean[, c("Latitude.x", "Longitude.x")]

# Define conditioning variables (climate + structure)
condition_vars <- env_data_clean[, c("bio_01.x", "bio_02.x", "bio_03.x", "bio_04.x",
                                     "bio_05.x", "bio_06.x", "bio_07.x", "bio_08.x",
                                     "bio_09.x", "bio_10.x", "bio_11.x", "bio_12.x",
                                     "bio_13.x", "bio_14.x", "bio_15.x", "bio_16.x",
                                     "bio_17.x", "bio_18.x", "bio_19.x",
...
# Run partial RDA: genotype ~ geography | climate + structure
rda_geography_partial <- rda(genotype_data_clean, geography_vars, condition_vars)

# View summary
summary(rda_geography_partial)

# Adjusted R²
RsquareAdj(rda_geography_partial)

# Permutation test for significance
anova(rda_geography_partial, permutations = 999)
plot(rda_geography_partial, scaling = 3)


#########################
#Structure ~ climate all variables + Latitude + Longtitude 
#########################

# Define response variable (Structure)
structure_vars <- env_data_clean[, c("EV1.x", "EV2.x", "EV3.x", "EV4.x")]

# Define explanatory variables (Climate + Geography)
explanatory_vars <- env_data_clean[, c("Latitude.x", "Longitude.x",
                                       "bio_01.x", "bio_02.x", "bio_03.x", "bio_04.x",
                                       "bio_05.x", "bio_06.x", "bio_07.x", "bio_08.x",
                                       "bio_09.x", "bio_10.x", "bio_11.x", "bio_12.x",
                                       "bio_13.x", "bio_14.x", "bio_15.x", "bio_16.x",
...
# Run RDA
rda_structure_climate_geo <- rda(structure_vars ~ ., data = explanatory_vars)

# Summary
summary(rda_structure_climate_geo)

# Adjusted R²
RsquareAdj(rda_structure_climate_geo)

# Permutation test
anova(rda_structure_climate_geo, permutations = 999)
...
# Define conditioning variables (Latitude + Longitude)
condition_vars <- env_data_clean[, c("Latitude.x", "Longitude.x")]

# Run partial RDA: Structure ~ Climate | Geography
rda_structure_climate_partial <- rda(structure_vars, climate_vars, condition_vars)

# Summary
summary(rda_structure_climate_partial)

# Adjusted R²
RsquareAdj(rda_structure_climate_partial)

# Permutation test
anova(rda_structure_climate_partial, permutations = 999)



#########################
# Structure ~ + Latitude + Longtitude | (climate all variables ) 
#########################
# Define geographic variables
geography_vars <- env_data_clean[, c("Latitude.x", "Longitude.x")]

# Define conditioning variables (Climate)
condition_vars <- env_data_clean[, c("bio_01.x", "bio_02.x", "bio_03.x", "bio_04.x",
                                     "bio_05.x", "bio_06.x", "bio_07.x", "bio_08.x",
                                     "bio_09.x", "bio_10.x", "bio_11.x", "bio_12.x",
                                     "bio_13.x", "bio_14.x", "bio_15.x", "bio_16.x",
...
# Run partial RDA: Structure ~ Geography | Climate
rda_structure_geo_partial <- rda(structure_vars, geography_vars, condition_vars)

# Summary
summary(rda_structure_geo_partial)

# Adjusted R²
RsquareAdj(rda_structure_geo_partial)

# Permutation test
anova(rda_structure_geo_partial, permutations = 999)
